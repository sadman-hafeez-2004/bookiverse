...
// ── Category grids ────────────────────────────────────────────
// Click on a category card → filters the books section below
function CategorySection({ allBooks, genres, activeTab, setActiveTab, setPage }) {
    ...
    <div style={{ marginBottom:32 }}>
      ...
      <div style={{ display:'flex', gap:14, overflowX:'auto', paddingBottom:10, WebkitOverflowScrolling:'touch' }}>
        {byGenre.map(({ genre, books }) => (
          <div
            key={genre}
            onClick={() => { setActiveTab(genre); setPage(1); }}
            style={{
              flexShrink:0, width:200,
              background: activeTab===genre ? 'var(--blue-fill)' : 'var(--surface)',
              border: activeTab===genre ? '1.5px solid var(--blue-btn)' : '0.5px solid var(--border)',
              borderRadius:10, padding:'12px 10px',
              cursor:'pointer', transition:'all 150ms',
            }}
          >
            <div style={{ fontSize:11, fontWeight:700, color:'var(--blue-btn)', letterSpacing:'0.06em', textTransform:'uppercase', marginBottom:10 }}>
              {genre}
            </div>
...
export default function Home() {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/books', { params:{ limit:60, sort:'popular' } }),
  ...
  const filtered = activeTab==='All' ? allBooks : allBooks.filter(b=>b.genre===activeTab);
  const sorted   = [...filtered].sort((a,b) => {
    if (sort==='popular')  return (b.collectionsCount||0)-(a.collectionsCount||0);
    if (sort==='topRated') return (b.averageRating||0)-(a.averageRating||0);
    return new Date(b.createdAt)-new Date(a.createdAt);
  });
  const visible = sorted.slice(0, page*20);
  ...
  return (
    <>
      <style>{`
        ...
        .ctabs { display:flex; gap:8px; overflow-x:auto; padding-bottom:6px; margin-bottom:20px; scrollbar-width:none; -webkit-overflow-scrolling:touch; }
        ...
        .bimg {
          ...
          position: relative; background: var(--blue-fill);
      ...
      <div className="hw">
        ...
        <div>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
            <div>
              <div style={{ fontSize:11, fontWeight:700, color:'var(--text-3)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:3 }}>All books</div>
              <h2 style={{ fontSize:18, fontWeight:700, color:'var(--text-1)' }}>
                {activeTab==='All' ? 'All books' : activeTab}
                <span style={{ fontSize:13, color:'var(--text-3)', fontWeight:400, marginLeft:8 }}>({sorted.length})</span>
              </h2>
            </div>
            ...
            <>
              ...
              {hasMore && (
                <div style={{ textAlign:'center', marginTop:28 }}>
                  <Btn variant="secondary" onClick={() => setPage(p=>p+1)}>Load more</Btn>
                </div>
              )}
            </>
          )}
