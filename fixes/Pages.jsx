...
import { Av, Btn, Input, Textarea, Spinner, Empty, Stars } from '../components/ui';
...
export function AuthorDetailPage() {
  const { id }   = useParams(); // FIX: useParams was missing from imports before
    ...
    <div className="container page">
      ...
      {loading ? <Spinner /> : (
        <>
          {tab === 'books' && (
            books.length === 0
              ? <Empty icon="📚" title="No books found" />
              : <div className="book-grid">{books.map(b => <BookCard key={b._id} book={b} />)}</div>
          )}
          {tab === 'authors' && (
            authors.length === 0
              ? <Empty icon="✍️" title="No authors found" />
              : (
          ...
          {tab === 'readers' && (
            users.length === 0
              ? <Empty icon="👤" title="No readers found" />
              : users.map(u => (
                <Link key={u._id} to={`/profile/${u._id}`} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '12px 0', borderBottom: '0.5px solid var(--border)' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--text-3)' }}>{u.collectedCount || 0} books · {u.followersCount || 0} followers</div>
                  </div>
                </Link>
              ))
          )}
