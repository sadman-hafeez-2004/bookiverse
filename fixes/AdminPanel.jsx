...
import { invalidateGenreCache } from '../hooks/useGenres';
  ...
  return (
    <>
      ...
      <div className="admin-wrap">
        ...
        <div style={{ display: 'flex', overflowX: 'auto', borderBottom: '0.5px solid var(--border)', marginBottom: 24, scrollbarWidth: 'none' }}>
...
function OverviewTab({ add }) {
  ...
  useEffect(() => {
    api.get('/admin/stats').then(r => setData(r.data)).catch(() => add('Failed to load stats', 'error'));
  }, []);
...
function UsersTab({ add }) {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      api.get('/admin/users', { params: { search, limit: 40 } })
        .then(r => setUsers(r.data.users))
        .catch(() => add('Failed to load users', 'error'));
    }, 300);
  ...
  const changeRole = async (id, role) => {
    try {
      ...
      // FIX: update role in state immediately
      setUsers(u => u.map(x => x._id === id ? { ...x, role: data.user.role } : x));
  ...
  const submitBan = async () => {
    try {
      await api.put(`/admin/users/${banForm.id}/ban`, { isBanned: true, banReason: banForm.reason });
      // FIX: update ban status in state immediately
      setUsers(u => u.map(x => x._id === banForm.id ? { ...x, isBanned: true, banReason: banForm.reason } : x));
      setShowBan(false);
      setBanForm({ id: '', reason: '' });
      add('User banned.');
    } catch (err) { add(err.response?.data?.message || 'Failed', 'error'); }
  };
  ...
  const unban = async (id) => {
    try {
      await api.put(`/admin/users/${id}/ban`, { isBanned: false });
      // FIX: update ban status in state immediately
      setUsers(u => u.map(x => x._id === id ? { ...x, isBanned: false, banReason: '' } : x));
      add('User unbanned.');
    } catch (err) { add(err.response?.data?.message || 'Failed', 'error'); }
  };
  ...
  const del = async (id, name) => {
    ...
    try {
      ...
      // FIX: remove from state immediately
          ...
          <tbody>
            {users.map(u => (
              <tr key={u._id}>
                ...
                <td>{u.isBanned ? <span className="badge-inactive">Banned</span> : <span className="badge-active">Active</span>}</td>
...
function BooksTab({ add }) {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      api.get('/admin/books', { params: { search, limit: 40 } })
        .then(r => setBooks(r.data.books))
        .catch(() => add('Failed to load books', 'error'));
    }, 300);
    return () => clearTimeout(t);
  }, [search]);
  ...
  const toggleFeature = async (id, current) => {
    try {
      ...
      // FIX: update featured flag immediately
      setBooks(b => b.map(x => x._id === id ? { ...x, isFeatured: data.book.isFeatured } : x));
      add(data.message);
    } catch (err) { add(err.response?.data?.message || 'Failed', 'error'); }
  };
...
function AuthorsTab({ add }) {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      api.get('/admin/authors', { params: { search, limit: 40 } })
        .then(r => setAuthors(r.data.authors))
        .catch(() => add('Failed to load authors', 'error'));
    }, 300);
    return () => clearTimeout(t);
  }, [search]);
  ...
  const startEdit = (a) => {
    setEditing(a._id);
    setForm({ name: a.name, bio: a.bio || '', nationality: a.nationality || '' });
  };

  const saveEdit = async () => {
    if (!form.name.trim()) return add('Name is required', 'error');
    try {
      const { data } = await api.put(`/admin/authors/${editing}`, form);
      // FIX: update author in state immediately
      setAuthors(a => a.map(x => x._id === editing ? { ...x, ...data.author } : x));
      setEditing(null);
      add('Author updated!');
    } catch (err) { add(err.response?.data?.message || 'Failed', 'error'); }
  };
  ...
  const del = async (id, name) => {
    ...
    try {
      ...
      add('Author deleted.');
    } catch (err) { add(err.response?.data?.message || 'Failed to delete author', 'error'); }
  };
                ...
                <td>
                  {editing === a._id ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      <input className="input" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} style={{ padding: '4px 8px', fontSize: 13 }} placeholder="Name" />
...
function ReviewsTab({ add }) {
  ...
  useEffect(() => {
    api.get('/admin/reviews', { params: { page, limit: 20 } })
      ...
      .catch(() => add('Failed to load reviews', 'error'));
  }, [page]);
      ...
      <div style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 10, overflow: 'auto' }}>
        <table className="admin-table">
          ...
          <tbody>
            {reviews.map(r => (
              <tr key={r._id}>
                <td style={{ fontWeight: 600 }}>{r.user?.username || '—'}</td>
...
function GenresTab({ add }) {
  ...
  // FIX: load genres on mount with proper loading state
  useEffect(() => {
    setLoading(true);
    api.get('/admin/genres')
      .then(r => setGenres(r.data.genres))
      .catch(() => add('Failed to load genres', 'error'))
  ...
  const create = async () => {
    ...
    try {
      ...
      // FIX: add genre to state immediately AND invalidate cache so Home/Upload update
      setGenres(g => [...g, data.genre]);
      invalidateGenreCache();
      setNewName('');
      add(`Genre "${data.genre.name}" added!`);
    } catch (err) { add(err.response?.data?.message || 'Failed to add genre', 'error'); }
  };

  const save = async (id) => {
    if (!editVal.trim()) return;
    try {
      const { data } = await api.put(`/admin/genres/${id}`, { name: editVal.trim() });
      // FIX: update genre name in state immediately AND invalidate cache
      setGenres(g => g.map(x => x._id === id ? { ...x, name: data.genre.name } : x));
      invalidateGenreCache();
      setEditing(null);
      add('Genre updated!');
    } catch (err) { add(err.response?.data?.message || 'Failed to update genre', 'error'); }
  };
  ...
  const del = async (id, name) => {
    ...
    try {
      ...
      // FIX: remove from state immediately AND invalidate cache
      setGenres(g => g.filter(x => x._id !== id));
      invalidateGenreCache();
      add('Genre deleted.');
    } catch (err) { add(err.response?.data?.message || 'Cannot delete this genre', 'error'); }
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        ...
        />
        <button className="ab ab-blue" onClick={create} style={{ padding: '9px 18px', fontSize: 14 }}>
          Add genre
        </button>
      </div>

      {loading ? (
        <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-3)' }}>Loading genres…</div>
      ) : genres.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-3)' }}>No genres yet. Add one above!</div>
      ) : (
            ...
            <div key={g._id} style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 8, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
              {editing === g._id ? (
                ...
                <>
                  ...
                  {g.isDefault && (
                    <span style={{ fontSize: 10, color: 'var(--text-3)', background: 'var(--surface-alt)', padding: '2px 6px', borderRadius: 10 }}>default</span>
                  )}
                  {!g.isDefault && (
                    <>
...
function AnnouncementsTab({ add }) {
  ...
  useEffect(() => {
    ...
    api.get('/admin/announcements')
      ...
      .catch(() => add('Failed to load announcements', 'error'))
      .finally(() => setLoading(false));
  }, []);
  ...
  const create = async () => {
    ...
    try {
      ...
      // FIX: add to top of list immediately
  ...
  const toggle = async (id) => {
    try {
      ...
      // FIX: update active status immediately
      setAnns(a => a.map(x => x._id === id ? { ...x, active: data.announcement.active } : x));
      add(data.message);
    } catch (err) { add(err.response?.data?.message || 'Failed', 'error'); }
  };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {anns.map(a => (
            <div key={a._id} style={{
              ...
              borderRadius: 8, padding: '14px 16px', opacity: a.active ? 1 : 0.5,
            }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                    <span style={{
                      ...
                      background: a.type === 'info' ? 'var(--blue-fill)' : a.type === 'warning' ? '#FEF3C7' : 'var(--green-50)',
                      color: a.type === 'info' ? 'var(--blue-text)' : a.type === 'warning' ? '#92400E' : 'var(--green-800)',
                    }}>{a.type}</span>
