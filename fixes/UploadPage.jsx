...
export function UploadPage() {
  ...
  const { genres, refresh } = useGenres(); // FIX: grab refresh to call after genre operations
  ...
  useEffect(() => {
    api.get('/authors', { params: { limit: 500 } }).then(({ data }) => setAllAuthors(data.authors));
  }, []);
  ...
  const checkDuplicate = (name) => {
    ...
    setDupWarning(found ? `"${found.name}" already exists. Select them in the Book tab instead.` : '');
  };
  ...
  const submitBook = async (e) => {
    ...
    try {
      ...
      // FIX: update author booksCount locally after upload
      ...
      setBookErr(err.response?.data?.message || 'Upload failed.');
  ...
  const submitAuthor = async (e) => {
    e.preventDefault();
    if (dupWarning) return add('Author already exists. Use the Book tab to select them.', 'error');
  ...
  return (
    <>
      <style>{`
        ...
        .up-err  { font-size:13px; color:var(--rose-800); }
          ...
          <form onSubmit={submitBook} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
              ...
              <div className="cover-box" onClick={() => coverRef.current.click()}>
                {coverPrev
                  ? <img src={coverPrev} alt="cover" />
                  : <><div className="plus">+</div><div className="hint">Cover image</div></>
                }
              </div>
            ...
            {/* FIX: genre dropdown uses live genres from DB */}
            <div className="up-field">
            ...
            <div className="up-field">
              ...
              {selectedAuthor ? (
                <div className="author-chip">
                  ...
                  <span style={{ flex: 1, fontSize: 14, fontWeight: 600, color: 'var(--blue-text)' }}>
                    {selectedAuthor.name}
                  </span>
                  <button type="button" onClick={clearAuthor} style={{
                    background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, color: 'var(--text-3)', padding: 0, lineHeight: 1,
                  }}>✕</button>
                </div>
              ) : (
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button type="button" onClick={() => setTab('author')} style={{
                    ...
                    fontWeight: 500, whiteSpace: 'nowrap', fontFamily: 'inherit', transition: 'all 150ms',
                  }}>+ New</button>
                </div>
              )}
            </div>
            ...
            <div className="up-field">
              ...
              <input className="up-input" type="number" placeholder="2024"
        ...
        {tab === 'author' && (
          <form onSubmit={submitAuthor} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div className="photo-circle" onClick={() => photoRef.current.click()}>
              {photoPrev
                ? <img src={photoPrev} alt="author" />
                : <><div className="plus">+</div><div className="hint">Photo</div></>
              }
            </div>
            ...
            <div className="up-field">
              ...
              <input className="up-input" placeholder="e.g. Matt Haig" value={aForm.name}
                onChange={e => { setAForm(p => ({ ...p, name: e.target.value })); checkDuplicate(e.target.value); }}
                required />
            ...
            <div className="up-field">
              ...
              <textarea className="up-input up-textarea" placeholder="Tell us about this author…"
                value={aForm.bio} onChange={e => setAForm(p => ({ ...p, bio: e.target.value }))}
                style={{ minHeight: 120 }} />
            </div>
            ...
            <p style={{ textAlign: 'center', fontSize: 13, color: 'var(--text-3)' }}>
              After saving, you'll be taken back to the book form to select this author.
            </p>
          </form>
        )}
      </div>
